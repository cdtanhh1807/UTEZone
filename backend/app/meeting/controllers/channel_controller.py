...
class ChannelWebSocketManager:
    ...
    async def broadcast(self, channel_id: str, message: dict):
        ...
        else: print(f"[BROADCAST] channel {channel_id} not in active_connections")
...
async def send_message(room_id: str, req: SendMessageRequest, request: Request, current_user: dict = Depends(get_current_user)):
    email = current_user["sub"]
    token = _extract_token(request)
    full_name = await get_fullname_from_api(email, token)
    chatroom = await channel_service.get_chat_room(room_id)
...
async def upload_chat_file(
    ...
    # Kiểm tra quyền: user phải là member của channel chứa room này
    chatroom = await channel_service.get_chat_room(room_id)
    if not chatroom:
        raise HTTPException(status_code=404, detail="Chat room không tồn tại")
